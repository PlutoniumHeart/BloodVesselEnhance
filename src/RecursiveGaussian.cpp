#ifndef RECURSIVEGAUSSIAN_CPP
#define RECURSIVEGAUSSIAN_CPP
...
template <class T1,class T2>
void CFilter<T1,T2>::SetDim(int w, int h, int d)
{
    width = w;
    height = h;
    depth = d;
...
template <class T1, class T2>
void CFilter<T1,T2>::SetInput(T1* input)
{
    int i = 0;
    delete [] h_input;
...
template <class T1, class T2>
int CFilter<T1, T2>::RecursiveGaussian(int xDirection, int yDirection, int zDirection,
...
template <class T1, class T2>
int CFilter<T1, T2>::RecursiveGaussianX(int* d_src, int* d_dest, int order, float sigma)
{
            ...
            for(j=0;j<height;j++)
            {
                for(i=0;i<width;i++)
                {
                    ...
                    if(i == width-1)
                    {
                        ...
                        out = (float)M11*(d_dest[height*width*k+height*j+i]-up) +
                                     M12*(d_dest[height*width*k+height*j+i-1]-up) +
                                     M13*(d_dest[height*width*k+height*j+i-2]-up)+vp;
                        
                        outF1 = (float)M21*(d_dest[height*width*k+height*j+i]-up) +
                                       M22*(d_dest[height*width*k+height*j+i-1]-up) +
                                       M23*(d_dest[height*width*k+height*j+i-2]-up)+vp;
                        
                        outF2 = (float)M31*(d_dest[height*width*k+height*j+i]-up) +
                                       M32*(d_dest[height*width*k+height*j+i-1]-up) +
                                       M33*(d_dest[height*width*k+height*j+i-2]-up)+vp;
            ...
            for(j=0;j<height;j++)
            {
                for(i=0;i<width;i++)
                {
                    ...
                    if(i == width-1)
                    {
                        ...
                        float up = (d_src[height*width*k+height*j+i] -
                                    d_src[height*width*k+height*j+i-1])/2.0*(1.0+b1+b2+b3);////
                        float vp = (float)up/(1.0+b1+b2+b3);
                        out = (float)M11*(d_dest[height*width*k+height*j+i]-up) +
                                     M12*(d_dest[height*width*k+height*j+i-1]-up) +
                                     M13*(d_dest[height*width*k+height*j+i-2]-up)+vp;
...
template <class T1, class T2>
int CFilter<T1, T2>::RecursiveGaussianY(int* d_src, int* d_dest, int order, float sigma)
{
    //const float nsigma = sigma < 0.5f ? 0.5f : sigma;
    float q = 0, m0 = 1.16680, m1 = 1.10783, m2 = 1.40586;
    float m1Sqr = m1*m1, m2Sqr = m2*m2;
            ...
            for(i=0;i<width;i++)
            {
                for(j=0;j<height;j++)
                {
                    ...
                    if(j == height-1)
                    {
                        ...
                        float vp = (float)up/(1.0+b1+b2+b3);
                        out = (float)M11*(d_dest[height*width*k+height*j+i]-up) +
                                     M12*(d_dest[height*width*k+height*(j-1)+i]-up) +
                                     M13*(d_dest[height*width*k+height*(j-2)+i]-up)+vp;
                        
                        outF1 = (float)M21*(d_dest[height*width*k+height*j+i]-up) +
                                       M22*(d_dest[height*width*k+height*(j-1)+i]-up) +
                                       M23*(d_dest[height*width*k+height*(j-2)+i]-up)+vp;
                        
                        outF2 = (float)M31*(d_dest[height*width*k+height*j+i]-up) +
                                       M32*(d_dest[height*width*k+height*(j-1)+i]-up) +
                                       M33*(d_dest[height*width*k+height*(j-2)+i]-up)+vp;
            ...
            for(i=0;i<width;i++)
            {
                for(j=0;j<height;j++)
                {
                    ...
                    if(j == height-1)
                    {
                        ...
                        float up = (d_src[height*width*k+height*j+i] -
                                    d_src[height*width*k+height*(j-1)+i])/2.0*(1.0+b1+b2+b3);
                        float vp = (float)up/(1.0+b1+b2+b3);
                        out = (float)M11*(d_dest[height*width*k+height*j+i]-up) +
                                     M12*(d_dest[height*width*k+height*(j-1)+i]-up) +
                                     M13*(d_dest[height*width*k+height*(j-2)+i]-up)+vp;
            ...
            for(j=0;j<height;j++)
            {
                for(k=0;k<depth;k++)
                {
                    ...
                    if(k == depth-1)
                    {
                        ...
                        float vp = (float)up/(1.0+b1+b2+b3);
                        out = (float)M11*(d_dest[height*width*k+height*j+i]-up) +
                                     M12*(d_dest[height*width*(k-1)+height*j+i]-up) +
                                     M13*(d_dest[height*width*(k-2)+height*j+i]-up)+vp;
                        
                        outF1 = (float)M21*(d_dest[height*width*k+height*j+i]-up) +
                                       M22*(d_dest[height*width*(k-1)+height*j+i]-up) +
                                       M23*(d_dest[height*width*(k-2)+height*j+i]-up)+vp;
                        
                        outF2 = (float)M31*(d_dest[height*width*k+height*j+i]-up) +
                                       M32*(d_dest[height*width*(k-1)+height*j+i]-up) +
                                       M33*(d_dest[height*width*(k-2)+height*j+i]-up)+vp;
            ...
            for(j=0;j<height;j++)
            {
                for(k=0;k<depth;k++)
                {
                    ...
                    if(k == depth-1)
                    {
                        ...
                        float up = (d_src[height*width*k+height*j+i] -
                                    d_src[height*width*(k-1)+height*j+i])/2.0*(1.0+b1+b2+b3);;
                        float vp = (float)up/(1.0+b1+b2+b3);
                        out = (float)M11*(d_dest[height*width*k+height*j+i]-up) +
                                     M12*(d_dest[height*width*(k-1)+height*j+i]-up) +
                                     M13*(d_dest[height*width*(k-2)+height*j+i]-up)+vp;
...
template <class T1, class T2>
int CFilter<T1, T2>::iDivUp(int a, int b)
{
...
template <class T1, class T2>
void CFilter<T1, T2>::CudaRecursiveGaussian(int xDirection, int yDirection, int zDirection,
                                            T1* d_src, T2* d_dest,/* int order, */float sigma)
{
    //int controle = 0;
    int size = width*height*depth;
...
template <class T1, class T2>
int CFilter<T1, T2>::CudaRecursiveGaussianX(int* d_src, int* d_dest, int order, float sigma)
{
    int size = width*height*depth;
    ...
    Call_d_recursiveGaussianX(d_src, d_dest, width, height, depth,
                              b0, b1, b2, b3, B, order, n, nthread,
                              M11, M12, M13, M21, M22, M23, M31, M32, M33);
    return 0;
}

template <class T1, class T2>
int CFilter<T1, T2>::CudaRecursiveGaussianY(int* d_src, int* d_dest, int order, float sigma)
{
    int size = width*height*depth;
    ...
    Call_d_recursiveGaussianY(d_src, d_dest, width, height, depth,
                              b0, b1, b2, b3, B, order, n, nthread,
                              M11, M12, M13, M21, M22, M23, M31, M32, M33);
    return 0;
}
...
template <class T1, class T2>
int CFilter<T1, T2>::CudaRecursiveGaussianZ(int* d_src, int* d_dest, int order, float sigma)
{
    int size = width*height*depth;
    ...
    Call_d_recursiveGaussianZ(d_src, d_dest, width, height, depth,
                              b0, b1, b2, b3, B, order, n, nthread,
                              M11, M12, M13, M21, M22, M23, M31, M32, M33);
    return 0;
}
...
template <class T1, class T2>
void CFilter<T1, T2>::ToHost(T2 *d_dest)
{
    int s = 1;
    if(!flag)
        s = 0;
...
template <class T1, class T2>
void CFilter<T1, T2>::SobelFilter(T1* in, T2* out)
{
    SetInput(in);
    int i = 0, j = 0, k = 0;
    int ul,um,ur,
...
template <class T1, class T2>
void CFilter<T1, T2>::CudaSobelFilter(T1* d_src, T2* d_dest)
{
    int size = width*height*depth;
    SetInput(d_src);
...
template <class T1, class T2>
void CFilter<T1, T2>::CudaSobelNew(T1* d_src, T2* d_dest)
{
    //int size = width*height*depth;
    int* data = NULL;
    SetInput(d_src);
...
//}
template <class T1, class T2>
void CFilter<T1, T2>::RecursiveGaussianFilter(int xDirection, int yDirection, int zDirection,
                                              T1* d_src, T2* d_dest, /*int order, */float sigma)
{
    bool cudaFlag = false;
    int deviceCount;
...
template <class T1, class T2>
void CFilter<T1, T2>::SobelEdgeDetect(T1* d_src, T2* d_dest)
{
    bool cudaFlag = false;
    int deviceCount;
    cudaGetDeviceCount(&deviceCount);
